...
function barthelme_admin() {
  if ( isset($_REQUEST['saved']) && $_REQUEST['saved'] ) { ?><div id="message1" class="updated fade"><p><?php printf(__('Barthelme theme options saved. <a href="%s">View site.</a>', 'mukti'), bloginfo('url') . '/'); ?></p></div><?php }
  if ( isset($_REQUEST['reset']) && $_REQUEST['reset'] ) { ?><div id="message2" class="updated fade"><p><?php _e('Barthelme theme options reset.', 'mukti'); ?></p></div><?php } ?>
...
<div class="wrap" id="barthelme-options">
  ...
  <form action="<?php echo _wp_specialchars( $_SERVER['REQUEST_URI'] ) ?>" method="post">
    ...
    <p class="submit">
      <input name="save" type="submit" value="<?php _e('Save Options', 'mukti'); ?>" tabindex="26" accesskey="S" />  
  ...
  <form action="<?php echo _wp_specialchars( $_SERVER['REQUEST_URI'] ) ?>" method="post">
    ...
    <p class="submit">
      <input name="reset" type="submit" value="<?php _e('Reset Options', 'mukti'); ?>" onclick="return confirm('<?php _e('Click OK to reset. Any changes to these theme options will be lost!', 'mukti'); ?>');" tabindex="27" accesskey="R" />
